 ...
 * Класс таблицы news.
 */
class NewsTableNews extends JTable
{
	/**
	 * Конструктор.
	 *
	...
	public function publish($pks = null, $state = 1, $userId = 0)
	{
		...
		foreach ($pks as $pk)
		{
			...
			if (!$this->load($pk))
			{
				throw new RuntimeException(JText::_('COM_NEWS_TABLE_ERROR_RECORD_LOAD'), 500);
			}
			...
			if (!$this->store())
			{
				throw new RuntimeException(JText::_('COM_NEWS_TABLE_ERROR_RECORD_STORE'), 500);
			}
		}
	...
	protected function _getAssetName()
	{
		...
		return 'com_news.message.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId($table = null, $id = null)
	{
		...
		if (($this->catid) && !empty($this->catid))
		{
			...
			$assetParent->loadByName('com_news.category.' . (int) $this->catid);
		}
		else
		{
			// В качестве родительского ассета записи выступает компонент.
			$assetParent->loadByName('com_news');
		}
